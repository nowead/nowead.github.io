...
지난 [Ep03]에서 Vulkan의 큰 그림을 봤다. 하지만 여전히 "Command Buffer는 Buffer인데 왜 데이터라고 안 하지?", "Index Buffer는 또 뭐야?" 같은 의문이 꼬리를 문다.
...
프로그램이 시작될 때 **"딱 한 번 만들어서 계속 쓰는"** 녀석들이다. 하드웨어와 소프트웨어의 뼈대를 담당한다.
